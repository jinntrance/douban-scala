...
class MovieTest extends BaseTest {
  ...
  test("tesing movie postReview") {
    prettyJSON(Movie.postReview(new MovieReviewPosted(movieId, "Test", content, 5)))
  }
  test("tesing movie delete review") {
    prettyJSON(Movie.deleteReview(reviewId))
  }
  test("tesing movie update review") {
    prettyJSON(Movie.updateReview(new MovieReviewPosted(movieId, "Test", content)))
  }
}
