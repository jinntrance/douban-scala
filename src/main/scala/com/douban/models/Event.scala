...
object Event extends API[Event] with CommentTrait[Event] with DiscussionTrait[Event] {
   ...
   */
  def participate(eventId: String, p: ParticipateDate = null) = post(participantsUrl.format(eventId), p,withResult = false)
   ...
   * 感兴趣
   */
  def wish(eventId: String) = post(wishersUrl.format(eventId), null,withResult = false)
